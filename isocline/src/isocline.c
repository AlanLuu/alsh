...
ic_public char* ic_readline(const char* prompt_text, bool *ctrlC) 
{
  ic_env_t* env = ic_get_env();
  if (env == NULL) return NULL;
  if (!env->noedit) {
    // terminal editing enabled
    return ic_editline(env, prompt_text, ctrlC);   // in editline.c
  } 
  else {
...
ic_public char* ic_readline_ex(const char* prompt_text,
                                ic_completer_fun_t* completer, void* completer_arg,
                                 ic_highlight_fun_t* highlighter, void* highlighter_arg, bool *ctrlC )
{
  ic_env_t* env = ic_get_env(); if (env == NULL) return NULL;
  // save previous
  ...
  char* res = ic_readline(prompt_text, ctrlC);
  // restore previous
